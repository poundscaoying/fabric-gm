...
chaincodeInvoke_test () {
	PEER=$1
	setGlobals $PEER
	# while 'peer chaincode' command can get the orderer endpoint from the peer (if join was successful),
	# lets supply it directly as we know it using the "-o" option
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -c '{"Args":["invoke","a","b","1"]}' >&log.txt
	else
...
testInvoke () {
	...
	do
		#sleep 0.15
		chaincodeInvoke_test 0
		let "Succ++"
	done

	echo "TEST INVOKE RECORD:" >> file.txt
	echo "NUM:$NUM" >> file.txt
	#get Succ
	echo "invoke successful transactions:$Succ" >> file.txt

	#get time 
	endtime=`date +'%s%N'`
	time_ms=$(((endtime-starttime)/1000000))
	echo "本次运行时间： "$time_ms"ms" >> file.txt

	#get tps
	tps=$(((Succ*100000)/time_ms))
	echo "tps*100: $tps" >> file.txt
...
testQuery () {
	...
	echo "NUM:$NUM" >> file.txt
	#get Succ
	echo "query successful transactions:$Succ" >> file.txt
...
testInvoke 50
echo
echo "=====================       finish test invoke scripts            ===================== "
echo "=====================                                             ===================== "
echo
...
testQuery 50
echo
echo "=====================       finish test query scripts            ===================== "
echo "=====================                                            ===================== "
echo
